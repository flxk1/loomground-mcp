...
| `privacy_scan(text, mode="standard", destination="external_llm", redaction_mode="redact", min_confidence="medium", audit_log_path?, tenant_id="", user_id="", hash_salt="")` | privacy-shield | `scan` | raw text → local span findings, clean overlay and source-classification egress verdict |
