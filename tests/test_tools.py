...
def test_loomground_catalogue():
    ...
    assert r["source"] == {"repo": "https://github.com/flxk1/loomground", "commit": "ba48b40540bb8788b92d03064ce6adc710139c0b"}
    names = [x["repo"] for x in r["repos"]]
    assert len(names) == 38 and names[0] == "loomground" and "loomground-mcp" in names
...
def test_loomground_releases():
    ...
    assert r["source"] == {"repo": "https://github.com/flxk1/loomground", "commit": "ba48b40540bb8788b92d03064ce6adc710139c0b"}
    assert len(r["repos"]) == 37 and len(r["edges"]) == 67 and len(r["accepted"]) == 18 and r["skipped"] == ["RVND"]
    assert sum(1 for x in r["repos"].values() if x["tag"]) == 25 and all(set(x) == {"version", "tag", "commit", "package", "pypi", "family", "tools", "skills"} for x in r["repos"].values())
