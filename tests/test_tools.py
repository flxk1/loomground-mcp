...
def test_loomground_catalogue():
    ...
    assert r["source"] == {"repo": "https://github.com/flxk1/loomground", "commit": "d7144d75f901641af652e5a1842f67dd85dd8c87"}
...
def test_loomground_releases():
    ...
    assert [(a["consumer"], a["dependency"]) for a in one["accepted"]] == [("loomground-mcp", "loomground-workspace")]
    ps = next(e for e in one["edges"] if e["dependency"] == "privacy-shield")
    assert (ps["range"], ps["dev_pin_release"], ps["status"]) == (">=2,<3", "v2.1.1", "release")
