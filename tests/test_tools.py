...
def test_loomground_catalogue():
    ...
    assert r["source"] == {"repo": "https://github.com/flxk1/loomground", "commit": "67f45b8de67b618f2d018bdb4c415de033f6b8f3"}
...
def test_loomground_releases():
    ...
    assert r["source"] == {"repo": "https://github.com/flxk1/loomground", "commit": "67f45b8de67b618f2d018bdb4c415de033f6b8f3"}
    assert len(r["repos"]) == 42 and len(r["edges"]) == 77 and len(r["accepted"]) == 0 and r["skipped"] == []
    assert sum(1 for x in r["repos"].values() if x["tag"]) == 33 and all(set(x) == {"version", "tag", "commit", "package", "pypi", "family", "tools", "skills"} for x in r["repos"].values())
