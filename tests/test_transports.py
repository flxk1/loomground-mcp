...
def test_sse_endpoint_and_tool_list(server):
    ...
    assert len(names) == len(ALL) == 46 and env["result"]["trace"]["evaluation"]["transfer"]["verdict"] == "reserved"
...
def test_streamable_http_tool_and_prompt_list(server):
    ...
    assert len(tools) == len(ALL) == 46 and len(prompts) == PROMPTS == 26
...
def test_sse_token_gate(server):
    ...
    assert len(asyncio.run(go())) == len(ALL) == 46
...
def test_streamable_http_token_gate_from_env(server):
    ...
    assert len(asyncio.run(go(BEARER))) == len(ALL) == 46
